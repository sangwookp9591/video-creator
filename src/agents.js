...
입력의 user_prompts.creative_prompt가 있으면 그 의도와 맞는 Topic을 우선한다.
...
user_prompts.creative_prompt가 있으면 해당 관점에 필요한 사실을 우선 정리한다.
...
user_prompts.creative_prompt가 있으면 톤, 목적, 포함 메시지를 기획에 반영한다.
...
user_prompts.creative_prompt가 있으면 대본의 메시지, 톤, 스토리 방향에 반영한다.
...
user_prompts.character_prompt가 있으면 모든 Scene의 visual_goal에 같은 캐릭터 일관성을 반영한다.
user_prompts.mascot_image_prompt와 mascot_image_url이 있으면 마스코트가 등장해야 하는 장면에 시각 목표로 반영한다.`,
  outputSchema: obj({
    scenes: arr(obj({
...
user_prompts.character_prompt, user_prompts.mascot_image_prompt, user_prompts.mascot_image_url이 있으면 image_prompt/video_prompt에 명시적으로 포함해 캐릭터와 마스코트 일관성을 유지한다.
user_prompts.creative_prompt가 있으면 video_prompt와 style에 반영한다.`,
  outputSchema: obj({
    scenes: arr(obj({
  ...
  mock: (input) => ({
    ...
    output: {
      scenes: input.scenes.map((s) => ({
        id: s.id,
        image_prompt: [
          `tech scene: ${s.visual_goal}`,
          input.user_prompts?.character_prompt && `character: ${input.user_prompts.character_prompt}`,
          input.user_prompts?.mascot_image_prompt && `mascot: ${input.user_prompts.mascot_image_prompt}`,
          input.user_prompts?.mascot_image_url && `mascot image reference: ${input.user_prompts.mascot_image_url}`,
        ].filter(Boolean).join(", "),
        video_prompt: [
          "slow push-in",
          input.user_prompts?.creative_prompt && `creative direction: ${input.user_prompts.creative_prompt}`,
        ].filter(Boolean).join(", "),
...
user_prompts.creative_prompt가 있으면 제목/설명/태그에 반영한다.
